...
class UpcomingComponent extends StatefulWidget {
  ...
  const UpcomingComponent({Key key, this.onClick}) : super(key: key);
  @override
  _UpcomingComponentState createState() => _UpcomingComponentState();
}

class _UpcomingComponentState extends State<UpcomingComponent> {
  bool _isLoading = false;
  List<EventModel> _activities = [];
  String _userId;
  ...
  _loadData() async{
    ...
    _result = await NodeService().getEventsPending(_data);
    if(_result != null && _result['data']!=null){
      _sortData(_result['data']);
    }

    _result = await NodeService().getEventsStandBy(_data);
    if(_result != null && _result['data']!=null){
      _sortData(_result['data']);
    }
  ...
  void initState() {
    ...
    scrollController.addListener(() {
      ...
      if(mounted) {
        if (device_height-300 < currentScroll) {
        setState(() {
          isTop = false;
        });
      } else {
        ...
        return Scaffold(
         ...
         body: ModalProgressHUD(
          child: Builder(builder: (context) {
            ...
            _activities.sort((a, b) => a.startDT.toUtc()?.millisecondsSinceEpoch
                ?.compareTo(b.startDT.toUtc()?.millisecondsSinceEpoch));
            var _list = groupBy(_activities, (EventModel obj) => obj.startDT.month);
            return ListView.builder(
              padding: const EdgeInsets.only(bottom: 65),
              itemCount: _list.values.length,
              itemBuilder: (context, index) {
                final monthName =
                getMonthName(_list.keys.toList()[index]);
                return Container(
                  height: 210,
                              ...
                              children: _list.values.toList()[index].map<Widget>((f) {
                                return GestureDetector(
                                  ...
                                  onTap: () async{
                                    ...
                                    if(_result != null && _result['data']!= null){
                                      f = EventModel.fromJson(_result['data']);
                                      navigateToDetailScreen(context, f);
                                    }
                                    // Navigator.pushNamed(
  ...
  // get event status label function
  Widget generateLabel(ApplicantStatusType type) {
    switch (type) {
      case ApplicantStatusType.my:
        return Row(
          ...
          children: <Widget>[
            ...
            Stack(
              children: [
                SvgPicture.asset('assets/svg/tapped.svg', width: 20, height: 20,),
                Positioned(
                  top: 4,
                  left: 2,
